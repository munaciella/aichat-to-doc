...
//   // Delete the doc from firebase db
...
//   // ✅ Delete chat messages before deleting the document
//   console.log("--- Deleting chat history from Firebase... ---");
...
async function deleteOldMessages(userId: string, docId: string) {
  ...
  const oldMessages = await chatRef
    .where("createdAt", "<", new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)) // 30 days
    .get();
...
export async function deleteDocument(docId: string) {
  ...
  // ✅ First, delete only old chat messages (if any exist)
